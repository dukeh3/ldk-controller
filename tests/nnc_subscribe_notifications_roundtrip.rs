...
use ldk_controller::{
    clear_subscriptions, clear_usage_profiles, run_nwc_service, set_relay_pubkey,
...
async fn test_nnc_subscribe_notifications_roundtrip() -> Result<()> {
    ...
    // Grant control access with subscribe_notifications permission
    let owner_keys = Keys::generate();
    let mut control = HashMap::new();
    control.insert(
        "subscribe_notifications".to_string(),
    ...
    // Send subscribe_notifications control request
    let payload = json!({
        "method": "subscribe_notifications",
        "params": {
            "types": ["channel_opened"]
        }
    });
    let encrypted =
        nip04::encrypt(&controller_secret, &service_pubkey, payload.to_string())?;
    ...
    let decrypted =
        nip04::decrypt(&controller_secret, &service_pubkey, &response_event.content)?;
    let response: Value = serde_json::from_str(&decrypted)?;

    assert_eq!(response["result_type"], "subscribe_notifications");
    assert!(
