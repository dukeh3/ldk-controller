...
/// End-to-end test: send a NWC make_new_address request, expect a valid response.
#[tokio::test]
async fn test_nwc_make_new_address_roundtrip() -> Result<()> {
    ...
    let request_event = Request::make_new_address()
        .to_event(&uri)
        ...
        while let Some(notification) = notifications.next().await {
            if let ClientNotification::Event { event, .. } = notification {
                ...
                if event.kind == Kind::WalletConnectResponse && event.pubkey == service_pubkey {
                    ...
                    // Without LDK service, this should return an error (ldk service unavailable)
                    assert!(
