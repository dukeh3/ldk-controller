...
use nwc::nostr::nips::nip47::{MakeOfferRequest, Method, NostrWalletConnectUri, Request, Response};
...
/// End-to-end test: send a NWC make_offer request, expect a valid response.
#[tokio::test]
async fn test_nwc_make_offer_roundtrip() -> Result<()> {
    ...
    methods.insert(Method::MakeOffer, MethodAccessRule { access_rate: None });
    ...
    let params = MakeOfferRequest {
        amount: 10_000,
        description: "test offer".to_string(),
        expiry: None,
    };
    let request_event = Request::make_offer(params)
        .to_event(&uri)
