...
async fn control_set_channel_fees_requires_ldk() -> Result<()> {
    ...
    control.insert("set_channel_fees".to_string(), MethodAccessRule { access_rate: None });
    ...
    assert_eq!(response["result_type"], "set_channel_fees");
    // Without LDK service, should return error
    assert!(
        response["error"].is_object(),
        "expected error when no ldk service, got: {:?}",
        response
    );
