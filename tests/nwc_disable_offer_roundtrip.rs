...
use nwc::nostr::nips::nip47::{
    DisableOfferRequest, Method, NostrWalletConnectUri, Request, Response,
};
use std::collections::HashMap;
use std::time::Duration;

use ldk_controller::{
    clear_offers_for_testing, clear_usage_profiles, set_relay_pubkey, MethodAccessRule,
    UsageProfile,
};
...
/// End-to-end test: disable_offer for non-existent offer → NOT_FOUND.
#[tokio::test]
async fn test_nwc_disable_offer_not_found() -> Result<()> {
    ...
    let params = DisableOfferRequest {
        offer: "lno1nonexistent".to_string(),
    };
    let request_event = Request::disable_offer(params)
        .to_event(&uri)
