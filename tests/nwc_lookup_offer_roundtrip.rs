...
use nwc::nostr::nips::nip47::{
    LookupOfferRequest, Method, NostrWalletConnectUri, Request, Response,
};
...
/// End-to-end test: send a NWC lookup_offer request for non-existent offer, expect NOT_FOUND error.
#[tokio::test]
async fn test_nwc_lookup_offer_not_found() -> Result<()> {
    ...
    methods.insert(Method::LookupOffer, MethodAccessRule { access_rate: None });
    ...
    let params = LookupOfferRequest {
        offer: "lno1nonexistent".to_string(),
    };
    let request_event = Request::lookup_offer(params)
        .to_event(&uri)
        ...
        while let Some(notification) = notifications.next().await {
            if let ClientNotification::Event { event, .. } = notification {
                ...
                if event.kind == Kind::WalletConnectResponse && event.pubkey == service_pubkey {
                    ...
                    // Should get NOT_FOUND error since the offer doesn't exist
