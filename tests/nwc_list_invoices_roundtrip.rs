...
use nwc::nostr::nips::nip47::{
    ListInvoicesRequest, Method, NostrWalletConnectUri, Request, Response,
};
...
/// End-to-end test: send a NWC list_invoices request, expect an empty list (no LDK service).
#[tokio::test]
async fn test_nwc_list_invoices_roundtrip() -> Result<()> {
    ...
    let params = ListInvoicesRequest {
        from: None,
        until: None,
        limit: None,
        offset: None,
    ...
    let request_event = Request::list_invoices(params)
        .to_event(&uri)
        ...
        while let Some(notification) = notifications.next().await {
            if let ClientNotification::Event { event, .. } = notification {
                ...
                if event.kind == Kind::WalletConnectResponse && event.pubkey == service_pubkey {
                    ...
                    let list = response
                        .to_list_invoices()
                        .expect("Response was not a valid list_invoices");

                    assert!(list.invoices.is_empty());
                    break;
                }
            }
        }
