...
async fn control_get_network_node_returns_not_found() -> Result<()> {
    ...
    control.insert("get_network_node".to_string(), MethodAccessRule { access_rate: None });
    ...
    // Use a valid but unknown pubkey
    let unknown_pubkey = "02aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    let payload = json!({
        "method": "get_network_node",
        "params": { "pubkey": unknown_pubkey }
    });
    ...
    assert_eq!(response["result_type"], "get_network_node");
    // Without LDK service, we get "ldk service unavailable" error
    // With LDK service, we'd get NOT_FOUND for unknown pubkey
    assert!(
        response["error"].is_object(),
        "expected error for unknown/unavailable node, got: {:?}",
        response
    );
