...
async fn control_get_forwarding_history_returns_empty() -> Result<()> {
    ...
    control.insert("get_forwarding_history".to_string(), MethodAccessRule { access_rate: None });
    ...
    assert_eq!(response["result_type"], "get_forwarding_history");
    assert!(response["error"].is_null(), "expected no error, got: {:?}", response);
    let forwards = &response["result"]["forwards"];
    assert!(
        forwards.is_array(),
        "expected forwards array in result, got: {:?}",
        response["result"]
    );
    assert_eq!(
        forwards.as_array().unwrap().len(),
        0,
        "expected empty forwards array"
    );
