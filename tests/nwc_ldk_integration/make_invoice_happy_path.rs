...
async fn make_invoice_happy_path() -> Result<()> {
    ...
    if let Some(err) = response.error {
        panic!("make_invoice returned error: {:?}", err);
    }

    let result = response
        .to_make_invoice()
        .expect("response should decode as make_invoice");

    assert!(!result.invoice.is_empty(), "invoice should not be empty");
    assert_eq!(result.amount, Some(123_000));
    assert!(
        result
            .payment_hash
            .as_ref()
