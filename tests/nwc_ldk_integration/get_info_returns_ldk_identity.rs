...
async fn get_info_returns_ldk_identity() -> Result<()> {
    ...
    if let Some(err) = response.error {
        panic!("get_info returned error: {:?}", err);
    }

    let info = response
        .to_get_info()
        .expect("response should decode as get_info");

    assert_eq!(info.network, Some("regtest".to_string()));
    assert_eq!(info.pubkey, Some(expected_ldk_pubkey));
