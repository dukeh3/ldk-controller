...
async fn pay_invoice_zero_amount_returns_error() -> Result<()> {
    ...
    let ldk_cfg = LdkServiceConfig {
        ...
        ldk_storage_dir: unique_storage_dir("nwc-ldk-pay-invoice-zero-amount"),
    ...
    let request = Request::pay_invoice(PayInvoiceRequest {
        id: None,
        invoice: "lnbcrt1p0invalidinvoice".to_string(),
        amount: Some(0),
    });
    ...
    let err = response
        .error
        .expect("expected pay_invoice to fail for zero amount");
