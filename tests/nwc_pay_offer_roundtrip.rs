...
use nwc::nostr::nips::nip47::{Method, NostrWalletConnectUri, PayOfferRequest, Request, Response};
...
/// End-to-end test: send a NWC pay_offer request, expect a valid response.
#[tokio::test]
async fn test_nwc_pay_offer_roundtrip() -> Result<()> {
    ...
    methods.insert(Method::PayOffer, MethodAccessRule { access_rate: None });
    ...
    let params = PayOfferRequest {
        offer: "lno1test".to_string(),
        amount: Some(10_000),
        payer_note: None,
    };
    let request_event = Request::pay_offer(params)
        .to_event(&uri)
        ...
        while let Some(notification) = notifications.next().await {
            if let ClientNotification::Event { event, .. } = notification {
                ...
                if event.kind == Kind::WalletConnectResponse && event.pubkey == service_pubkey {
                    ...
                    // Without LDK service, this should return an error
                    assert!(
                        response.error.is_some(),
                        "Expected error without LDK service"
                    );
                    break;
                }
            }
